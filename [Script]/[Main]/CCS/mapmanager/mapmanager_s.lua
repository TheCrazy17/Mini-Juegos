...
function MapManager.startMap(map, sendToPlayers)
	...
	if getElementData(source, "gamemode") ~= "Freeroam" or getElementData(source, "gamemode") ~= "Hay" then
	...
	if sendToPlayers then
		...
		for i, player in ipairs(getPlayersAndSpectatorsInArena(source)) do
			...
			outputDebugString("[LOADER] Trigger para "..player.name)
...
function MapManager.fetchMaps(element)
	...
	MapTypes = {"Classic", "Cross", "Oldschool", "Shooter", "Hunter", "Modern", "Race", "Freeroam", "Linez", "Dynamic", "Maptest", "BattleRoyale", "Gungame"}
