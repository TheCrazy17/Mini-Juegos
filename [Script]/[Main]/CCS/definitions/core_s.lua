...
function Core.setupDefinitions(arenaElement)
	...
	elseif getElementData(arenaElement, "gamemode") == "Hay" then
		triggerEvent("onSetUpHayDefinitions", arenaElement)
	end

	if getElementData(arenaElement, "name") == "gungame" then
		triggerEvent("onLoadGungameDefinitions", arenaElement)
	end
end
...
function Core.joinArena(arena, isSpectator)
	...
	elseif getElementData(arenaElement, "gamemode") == "Hay" then
		...
		triggerClientEvent(source, "onClientSetUpHayDefinitions", root)
	...
	if getElementData(arenaElement, "name") == "gungame" then
		triggerClientEvent(source, "onClientLoadGungameDefinitions", root)
	end
...
function Core.leaveArena(quitType)
	...
	triggerClientEvent(source, "onClientUnloadGungameDefinitions", arenaElement)
	...
	if #getPlayersInArena(arenaElement) == 0 and not getElementData(source, "Spectator") then
		...
		triggerEvent("onUnloadGungameDefinitions", arenaElement)

		triggerEvent("onSetDownHayDefinitions", arenaElement)
